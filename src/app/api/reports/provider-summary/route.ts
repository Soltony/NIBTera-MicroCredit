...
async function getAggregatedLedgerEntries(
  providerId: string,
  timeframe: string,
  from: string | null,
  to: string | null,
  ...
  const aggregated = categories.reduce(
    (acc, cat) => ({ ...acc, [cat.toLowerCase()]: 0 }),
...
export async function GET(req: NextRequest) {
  ...
  if (!isSuperAdminOrRecon && user.loanProviderId !== providerId) {
    return NextResponse.json(
      {
        error: "Forbidden: You can only access reports for your own provider.",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const collections = await getAggregatedLedgerEntries(
      providerId,
      timeframe,
      from,
      to,
      "Debit",
      ["Received"],
      ["Principal", "Interest", "ServiceFee", "Penalty"]
    );
    ...
    // 3. Income Statement
    const accruedIncome = await getAggregatedLedgerEntries(
      providerId,
      timeframe,
      from,
      to,
      "Credit",
      ["Income"],
      ["Interest", "ServiceFee", "Penalty"]
    );
    const collectedIncome = await getAggregatedLedgerEntries(
      providerId,
      timeframe,
      from,
      to,
    ...
    const netRealizedIncome =
      (collectedIncome.interest || 0) +
      (collectedIncome.servicefee || 0) +
      (collectedIncome.penalty || 0);

    // 4. Fund Utilization
    // Use same calculation as Dashboard: sum of loan.loanAmount from non-reversed loans
    const provider = await prisma.loanProvider.findUnique({
      where: { id: providerId },
    });

    // Get all non-reversed loans for this provider and sum their loanAmount (same as Dashboard)
    const providerLoans = await prisma.loan.findMany({
      where: {
        product: { providerId },
        repaymentStatus: { not: "REVERSED" },
      },
    ...
    const totalDisbursedEver = providerLoans.reduce(
      (acc, loan) => acc + loan.loanAmount,
      0
    );

    const fundUtilization =
      provider && provider.startingCapital > 0
        ? (totalDisbursedEver / provider.startingCapital) * 100
        : 0;
    ...
    const borrowerIds = Array.from(
      new Set(overdueLoans.map((l) => l.borrowerId))
    );
    const phoneAccounts =
      borrowerIds.length > 0
        ? await prisma.phoneAccount.findMany({
            where: { phoneNumber: { in: borrowerIds }, isActive: true },
          })
        : [];
    const phoneMap = new Map(
      phoneAccounts.map((p) => [p.phoneNumber, p.accountNumber])
    );

    // Helper: extract borrower name from provisionedData payload
    const getBorrowerNameFromProvisioned = (
      pdRaw: string | undefined | null
    ) => {
    ...
    for (const loan of overdueLoans) {
      ...
      if (!byBorrower[borrowerKey]) {
        ...
        byBorrower[borrowerKey] = {
          ...
          borrowerName:
            derivedName || (loan.borrower ? loan.borrower.id || "" : ""),
          borrowerAccount: "",
      ...
      if (
        byBorrower[borrowerKey].maxDaysOverdue === undefined ||
        daysOverdue > byBorrower[borrowerKey].maxDaysOverdue
      ) {
      ...
      byBorrower[borrowerKey].principalOutstanding +=
        principalOutstandingForLoan;
      byBorrower[borrowerKey].interestOutstanding += interestOutstandingForLoan;
      byBorrower[borrowerKey].serviceFeeOutstanding +=
        serviceFeeOutstandingForLoan;
          ...
          if (pdRaw) {
            try {
              ...
              const candidate =
                ...
                null;
              if (candidate)
                byBorrower[borrowerKey].borrowerAccount = String(candidate);
            } catch (e) {
              // ignore parse errors
            }
          }
      ...
      providerBuckets[classification] =
        (providerBuckets[classification] || 0) + 1;
    ...
    return NextResponse.json({
      ...
      incomeStatement: {
        accrued: accruedIncome,
        collected: collectedIncome,
        net: netRealizedIncome,
      },
      fundUtilization,
      agingReport: {
