...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      try {
        const sampleProviders = await prisma.disbursementTransaction.findMany({
          where: { createdAt: { gte: subDays(new Date(), 90) } },
          select: { providerId: true, originalProviderId: true },
          take: 50,
        });
        const provSet = new Set(
          sampleProviders.map((p) => String(p.providerId || ""))
        );
        const origSet = new Set(
          sampleProviders.map((p) => String(p.originalProviderId || ""))
        );
      } catch (e) {
    ...
    const rows = await Promise.all(
      journalEntries.map(async (je) => {
        ...
        // Helper to check if a reference looks like an FT number (CBS transaction reference)
        const isFtNumber = (ref: string | null | undefined): boolean => {
          if (!ref) return false;
          // FT numbers typically start with 'FT' followed by digits/alphanumeric
          return /^FT[A-Z0-9]+$/i.test(ref);
        };

        try {
          // For repayments only: resolve the CBS transaction reference (FT number) from PaymentTransaction
          if (je.payment && loan?.id) {
            ...
            if (pending && pending.transactionId) {
              ...
              if (pt && pt.transactionId && isFtNumber(pt.transactionId)) {
                reference = pt.transactionId;
              } else if (pt && pt.transactionId) {
                // Even if not a typical FT format, use it if available
                reference = pt.transactionId;
              }
            }
            ...
            // 3) Last fallback: search PaymentTransaction by transactionId pattern matching
            // the loan's pending payments to find any FT reference
            if (reference === je.id) {
        ...
        if (loan && loan.borrowerId) {
          ...
          try {
            if (loan?.id) {
              foundMatch =
                disbursementTxs.find(
                  (d: any) => String(d.loanId) === String(loan.id)
                ) || null;
              if (foundMatch) {
              }
            }
          ...
          if (!foundMatch && borrowerAccount) {
            ...
            if (candidates.length > 0) {
              ...
              foundMatch =
                candidates.find(
                  (c: any) =>
                    Math.abs((c.amount || 0) - principalDisbursed) < 0.01
                ) || candidates[0];
            } else {
              try {
                // No candidates were found for this account; log sample provider candidates (if any)
                const providerCandidates = disbursementTxs.filter(
                  (d) =>
                    String(d.providerId) === String(provider?.id) ||
                    String(d.originalProviderId) === String(provider?.id)
                );
              } catch (e) {}
            }
          }
          ...
          if (!foundMatch && provider?.id) {
            ...
            if (providerCandidates.length > 0) {
              const closeByAmount = providerCandidates.filter(
                (c: any) =>
                  Math.abs((c.amount || 0) - principalDisbursed) < 0.01
              );
              const searchPool =
                closeByAmount.length > 0 ? closeByAmount : providerCandidates;
          ...
          if (foundMatch) {
            ...
            try {
              ...
              if (raw) {
                ...
                if (parsed) {
                  ...
                  const s = String(raw);
                  const m2 =
                    s.match(/status[_ ]?code\W*:?\W*(\d{3})/i) ||
                    s.match(/status\W*:?\W*([A-Za-z]+)/i);
                  if (m2) disbursementStatusText = m2[1];
                  else disbursementStatusText = s.slice(0, 200);
                }
              }
            } catch (e) {
              if (!disbursementStatusText && disbursementRawResponse)
                disbursementStatusText = String(disbursementRawResponse).slice(
                  0,
                  200
                );
            }
            ...
            try {
              if (
                ...
                Number(disbursementStatusCode) === 200
              ) {
                disbursementOutcome = "Success";
              } else if (disbursementStatusText) {
                const s = String(disbursementStatusText).toLowerCase();
                if (/success|completed|ok|200/.test(s))
                  disbursementOutcome = "Success";
                else if (
                  /fail|failed|error|decline|500|400|insufficient|minimum/.test(
                    s
                  )
                )
          ...
          try {
            if (
              ...
              if (/success|completed|ok|200/.test(s))
                ...
                /fail|failed|error|decline|500|400|insufficient|minimum/.test(s)
              )
                disbursementOutcome = "Failure";
            }
          } catch (e) {
