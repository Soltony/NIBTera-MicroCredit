...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.installmentId) {
      ...
      if (String(installment.loanId) !== String(loanId))
        throw new Error("Installment does not belong to loan");
      ...
      if (!installment.isActive) {
        throw new Error(
          "This installment is not active yet. Please repay the active installment first."
        );
      }
      ...
      const daysOverdue = Math.max(
        0,
        differenceInDays(paymentDate, installment.dueDate)
      );
        ...
        if (daysOverdue >= fromDay) {
          ...
          if (rule.type === "fixed") {
            ...
            penaltyForInstallment +=
              installment.amount * (value / 100) * daysToCalculate;
      ...
      const principalPaidSoFar = Math.max(
        0,
        (installment.paidAmount || 0) - penaltyPaidSoFar
      );
      const principalRemaining = Math.max(
        0,
        installment.amount - principalPaidSoFar
      );
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const journalEntry = await tx.journalEntry.create({
          data: {
            ...
            loanId,
            date: paymentDate,
            description: `Repayment for installment ${installment.installmentNumber} of loan ${loanId}`,
          },
        });
        ...
        if (penaltyToPay > 0 && penaltyReceivable && penaltyReceived) {
          ...
          if (!penaltyIncome)
            throw new Error("Penalty Income ledger account not configured");
          await tx.ledgerAccount.update({
        ...
        if (interestToPay > 0) {
          if (!interestReceivable || !interestReceived || !interestIncome)
            throw new Error("Interest ledger accounts not configured");
          await tx.ledgerAccount.update({
        ...
        if (taxToPay > 0) {
          if (!taxReceivable || !taxReceived)
            throw new Error("Tax ledger accounts not configured");
          await tx.ledgerAccount.update({
        ...
        const isFullyPaid =
          newPaidAmount >= installment.amount + penaltyForInstallment - 1e-9;
        ...
        await tx.loanInstallment.update({
          ...
          data: {
            ...
            status: isFullyPaid
              ? "Paid"
              : differenceInDays(paymentDate, installment.dueDate) > 0
              ? "Overdue"
              : "Pending",
        ...
        await tx.loan.update({
          where: { id: loanId },
          data: { repaidAmount: (loan.repaidAmount || 0) + paymentAmount },
        });
