  ...
  try {
    ...
    return providers.map((p) => ({
      ...
      products: p.products.map((prod) => ({
        ...
        serviceFee: safeJsonParse(prod.serviceFee, {
          type: "percentage",
          value: 0,
        }) as FeeRule,
        dailyFee: safeJsonParse(prod.dailyFee, {
          type: "percentage",
          value: 0,
        }) as FeeRule,
  ...
  try {
    ...
    return loans.map((loan) => ({
      ...
      product: {
        ...
        serviceFee: safeJsonParse(loan.product.serviceFee, {
          type: "percentage",
          value: 0,
        }),
        dailyFee: safeJsonParse(loan.product.dailyFee, {
          type: "percentage",
          value: 0,
          calculationBase: "principal",
        }),
        penaltyRules: safeJsonParse(loan.product.penaltyRules, []),
        requiredDocuments: safeJsonParse(
          loan.product.requiredDocuments,
          []
        ) as string[],
      },
        ...
        loan.installments?.map((i) => ({
          ...
          penaltyAmount: calculateInstallmentPenalty({
            ...
            principalOutstanding: Math.max(
              0,
              (i.amount || 0) - (i.paidAmount || 0)
            ),
            penaltyRules:
              (safeJsonParse(loan.product.penaltyRules, []) as any) || [],
...
export default async function LoanPage({
  ...
  if (
    !borrowerIdFromUrl ||
    String(borrowerIdFromUrl) !== String(ctx.borrowerId)
  ) {
  ...
  return (
    ...
    >
      <DashboardClient
        providers={providers}
        initialLoanHistory={loanHistory}
