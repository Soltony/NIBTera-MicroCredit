...
export function ConnectClient({ token, validationApiUrl }: ConnectClientProps) {
  ...
  useEffect(() => {
    const connectUser = async () => {
      // DEV-ONLY: Simulate successful login for local testing
      if (token === 'dev-mode-token') {
          setStatus('Redirecting (Dev Mode)...');
          router.replace(`/loan?borrowerId=borrower-123`);
          return;
      }
      ...
      try {
        ...
        // Use phone number as the borrowerId and redirect
        setStatus('Redirecting...');
        router.replace(`/loan?borrowerId=${phoneNumber}`);
