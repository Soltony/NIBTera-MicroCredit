...
async function getLoanHistory(borrowerId: string): Promise<LoanDetails[]> {
  try {
    ...
    return loans.map((loan) => {
      const parsedProduct = {
        ...loan.product,
        serviceFee: safeJsonParse(loan.product.serviceFee as string, {
          type: "percentage",
          value: 0,
        }),
        dailyFee: safeJsonParse(loan.product.dailyFee as string, {
          type: "percentage",
          value: 0,
      ...
      const { total: totalRepayable } = calculateTotalRepayable(
        loan as any,
        parsedProduct,
        taxConfigs,
        getAsOfDate()
