...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        console.log('[API][settings/providers] Received POST request with body:', JSON.stringify({ body: body, timestamp: new Date().toISOString() }));
        ...
        console.log('[API][settings/providers] Creating provider with data:', JSON.stringify({ ...logDetails, action: 'PROVIDER_CREATE_INITIATED', actorId: session.userId, timestamp: new Date().toISOString() }));
        ...
        const newProvider = await prisma.$transaction(async (tx) => {
            ...
            console.log(`[API][settings/providers] Created provider ${provider.id} inside transaction.`);
            ...
            console.log(`[API][settings/providers] Created ${accountsToCreate.length} ledger accounts for provider ${provider.id}.`);
        ...
        console.log('[API][settings/providers] Provider creation successful:', JSON.stringify({ ...successLogDetails, action: 'PROVIDER_CREATE_SUCCESS', actorId: session.userId, timestamp: new Date().toISOString() }));
        ...
        console.error('[API][settings/providers] Provider creation failed:', JSON.stringify({ ...failureLogDetails, action: 'PROVIDER_CREATE_FAILED', actorId: session.userId, timestamp: new Date().toISOString() }));
...
export async function PUT(req: NextRequest) {
    ...
    try {
        const body = await req.json();
        console.log('[API][settings/providers] Received PUT request with body:', JSON.stringify({ body: body, timestamp: new Date().toISOString() }));
        ...
        console.log('[API][settings/providers] Updating provider with data:', JSON.stringify({ ...logDetails, action: 'PROVIDER_UPDATE_INITIATED', actorId: session.userId, timestamp: new Date().toISOString() }));
        ...
        console.log('[API][settings/providers] Provider update successful:', JSON.stringify({ ...successLogDetails, action: 'PROVIDER_UPDATE_SUCCESS', actorId: session.userId, timestamp: new Date().toISOString() }));
        ...
        console.error('[API][settings/providers] Provider update failed:', JSON.stringify({ ...failureLogDetails, action: 'PROVIDER_UPDATE_FAILED', actorId: session.userId, timestamp: new Date().toISOString() }));
        return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
    }
}
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        ...
        console.log(`[API][settings/providers] Received DELETE request for provider ID: ${id}`);
        ...
        console.log('[API][settings/providers] Initiating provider deletion:', JSON.stringify({ ...logDetails, action: 'PROVIDER_DELETE_INITIATED', actorId: session.userId, timestamp: new Date().toISOString() }));
        ...
        console.log('[API][settings/providers] Provider deletion successful:', JSON.stringify({ ...successLogDetails, action: 'PROVIDER_DELETE_SUCCESS', actorId: session.userId, timestamp: new Date().toISOString() }));
        ...
        await createAuditLog({ actorId: session.userId, action: 'PROVIDER_DELETE_FAILED', entity: 'PROVIDER', entityId: id || undefined, details: failureLogDetails, ipAddress, userAgent });
         console.error('[API][settings/providers] Provider deletion failed:', JSON.stringify({ ...failureLogDetails, action: 'PROVIDER_DELETE_FAILED', actorId: session.userId, timestamp: new Date().toISOString() }));
