...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!superAppToken) {
            console.error("Super App authorization token is missing from the user session.");
        ...
        const signatureString = [
            ...
            `Key=${NIB_PAYMENT_KEY}`,
            `token=${superAppToken.substring(7)}`, // Extract token from "Bearer <token>"
        ...
        const payload = {
            ...
            callBackURL: CALLBACK_URL,
            companyName: COMPANY_NAME,
            token: superAppToken.substring(7), // Send the raw token
            transactionId: transactionId,
            transactionTime: transactionTime,
            signature: signature
        };
        ...
        const paymentResponse = await fetch(NIB_PAYMENT_URL, {
            ...
            headers: {
                ...
                // or a proxy, but we pass it here just in case.
