    ...
    return (
        <Card>
            ...
            <CardFooter className="flex justify-end gap-2">
                <Button variant="destructive" onClick={() => setIsDeleting(true)} disabled={isSaving || isDeleting || config.id.startsWith('new-') || isPending}>
                    {isDeleting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Trash2 className="mr-2 h-4 w-4" />}
                    Delete
                </Button>
                <Button onClick={handleSave} disabled={isSaving || isDeleting || isPending}>
                    {isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
                    {isPending ? 'Pending Approval' : 'Submit for Approval'}
                </Button>
            </CardFooter>
...
export function TaxSettingsClient({ initialTaxes }: { initialTaxes: TaxConfig[] }) {
    const [taxes, setTaxes] = useState<TaxConfig[]>(initialTaxes);
    ...
    const handleSave = async (taxToSave: TaxConfig, originalTax?: TaxConfig) => {
        ...
        try {
            ...
            if (isNew) {
                setTaxes(prev => prev.filter(t => t.id !== taxToSave.id));
            } else {
                setTaxes(prev => prev.map(t => t.id === taxToSave.id ? { ...t, status: 'PENDING_APPROVAL' } : t));
            }
    ...
    const handleDelete = async (taxToDelete: TaxConfig) => {
        ...
        try {
            ...
            toast({ title: "Deletion Submitted", description: 'Tax configuration deletion is pending approval.' });
             setTaxes(prev => prev.map(t => t.id === taxToDelete.id ? { ...t, status: 'PENDING_APPROVAL' } : t));
        } catch (error: any) {
             toast({ title: 'Error', description: error.message, variant: 'destructive' });
             throw error;
        }
