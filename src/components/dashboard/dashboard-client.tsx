...
export function DashboardClient({
  ...
  const { overallMaxLimit, totalBorrowed, availableToBorrow } = useMemo(() => {
    ...
    const outstandingPrincipal = unpaidLoans.reduce((acc, loan) => {
      ...
      if (
        Array.isArray((loan as any).installments) &&
        (loan as any).installments.length > 0
      ) {
        const instPrincipal = (loan as any).installments
          .filter((inst: any) => inst.status !== "Paid")
          .reduce(
            (sum: number, inst: any) =>
              sum + Math.max(0, (inst.amount || 0) - (inst.paidAmount || 0)),
            0
          );
  ...
  const activeLoansByProduct = useMemo(() => {
    ...
    return unpaidLoans.reduce((acc, loan) => {
      ...
      if (existing) {
        const existingActiveInst = Array.isArray(existing.installments)
          ? existing.installments.find((i: any) => i.isActive)
          : undefined;
        existingEffectiveDue = existingActiveInst?.dueDate
          ? new Date(existingActiveInst.dueDate)
