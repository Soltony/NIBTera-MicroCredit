...
export function DashboardClient({ providers, initialLoanHistory, taxConfigs }: DashboardClientProps) {
  ...
  const handleConfirmRepayment = async (amount: number) => {
    ...
    useEffect(() => {
        ...
        const checkOnce = async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    // keep polling until deadline (includes 404 while callback hasn't arrived)
                    return;
                }
                ...
                if (status === 'COMPLETED') {
                    ...
                    publishPaymentCompleted(data?.loanId ?? undefined);
                    router.refresh();
                    return;
                }
                ...
                // ignore transient network errors until deadline
            }
        };
    ...
    }, [isPollingPayment, pendingPaymentTxnId, pendingPaymentStartedAtMs, router, toast]);
